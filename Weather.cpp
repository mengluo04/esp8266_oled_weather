...
void Weather::UpDateWeatherNow(WeatherNowData *data) {
  ...
  HTTPClient http;
  String url = "http://192.168.5.7:6789/v7/weather/now?location=" + _reqLocation + "&key=" + _requserKey;
  if (http.begin(client, url)) {  // HTTP连接成功
    ...
    if (httpCode > 0) {                                                           // 错误返回负值
      ...
      data->ServerCode = http.errorToString(httpCode).c_str();
      data->LastUpdate = "-1";
      data->Temp = "N/A";
      data->FeelLike = "-1";
      data->Icon = ")";
...
void Weather::UpDateWeatherForecast(WeatherForecastData *data) {
  WiFiClient client;  //定义WiFi客户端
  HTTPClient http;
  String url = "http://192.168.5.7:6789/v7/weather/3d?location=" + _reqLocation + "&key=" + _requserKey + "&unit=" + _reqUnit + "&lang=" + _reqLang + "&gzip=n";
  if (http.begin(client, url)) {                                                  // HTTP连接成功
    int httpCode = http.GET();                                                    // 请求
    if (httpCode > 0) {                                                           // 错误返回负值
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {  // 服务器响应
      ...
      for (int i = 0; i < 3; i++) {
        data[i].ServerCode = http.errorToString(httpCode).c_str();
        data[i].LastUpdate = "-1";
        data[i].FxDate = "-1";
        data[i].TempMax = "-1";
        data[i].TempMin = "-1";
...
void Weather::UpDateAirQuality(WeatherNowData *data) {
  WiFiClient client;  //定义WiFi客户端
  HTTPClient http;
  String url = "http://192.168.5.7:6789/v7/air/now?location=" + _reqLocation + "&key=" + _requserKey + "&unit=" + _reqUnit + "&lang=" + _reqLang + "&gzip=n";
  if (http.begin(client, url)) {                                                 // HTTP连接成功
    int httpCode = http.GET();                                                    // 请求
    if (httpCode > 0) {                                                           // 错误返回负值
      if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {  // 服务器响应
...
String Weather::getMeteoconIcon(String cond_code) {
  ...
  if (cond_code == "306" || cond_code == "307" || cond_code == "310" || cond_code == "311" || cond_code == "312" || cond_code == "315" || cond_code == "316" || cond_code == "317") {
    return "R";
  }
  if (cond_code == "300" || cond_code == "301" || cond_code == "350" || cond_code == "351") {
    return "T";
  }
  if (cond_code == "400" || cond_code == "404" || cond_code == "406" || cond_code == "407" || cond_code == "408" || cond_code == "456" || cond_code == "457") {
    return "U";
  }
  ...
  if (cond_code == "304" || cond_code == "308" || cond_code == "312" || cond_code == "313" || cond_code == "318") {
    return "X";
  }
  if (cond_code == "101" || cond_code == "103" || cond_code == "151"|| cond_code == "153") {
    return "Y";
  }
  if (cond_code == "303") {
    return "0";
